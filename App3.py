...
def filter_dataframe(df, filters):
    ...
    if filters['disease_area'] and filters['disease_area'] != "Any":
        ...
        for col in filters['disease_cols']:
            ...
            debug_msgs.append(f"Column {col}: {mask.sum()} matches")

        d = d[disease_match]
        debug_msgs.append(f"After disease filtering: {len(d)} rows left")
    ...
    if filters['gender'] != "Any" and filters['gender_col']:
        ...
        debug_msgs.append(f"Gender filter removed {before - len(d)} rows")
    ...
    if filters['ethnicity'] != "Any" and filters['ethnicity_col']:
        ...
        debug_msgs.append(f"Ethnicity filter removed {before - len(d)} rows")
    ...
    if filters['age_col']:
        ...
        debug_msgs.append(f"Age filter removed {before - len(d)} rows")
    ...
    if filters['expertise_search'] and filters['expertise_col']:
        ...
        debug_msgs.append(f"Expertise search removed {before - len(d)} rows")
...
with u_col2:
    if st.button("+ Manage Data / Import JSON"):
        st.info("Import JSON action triggered.")
...
name_col = st.selectbox("Select column for Name (required)", columns)
email_col = st.selectbox("Select column for Email (required)", columns)

disease_cols = st.multiselect("Select ALL Disease / Condition columns", columns)

gender_col = st.selectbox("Select Gender column", ["-- None --"] + columns)
age_col = st.selectbox("Select Age column", ["-- None --"] + columns)
ethnicity_col = st.selectbox("Select Ethnicity column", ["-- None --"] + columns)
expertise_col = st.selectbox("Select Expertise / Keywords column", ["-- None --"] + columns)

# Normalize None selections
gender_col = None if gender_col == "-- None --" else gender_col
age_col = None if age_col == "-- None --" else age_col
ethnicity_col = None if ethnicity_col == "-- None --" else ethnicity_col
expertise_col = None if expertise_col == "-- None --" else expertise_col
...
# --- Filter ---
results, debug_info = filter_dataframe(df, filters)

with st.expander("🔧 Filter Debugging"):
